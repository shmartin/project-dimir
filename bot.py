...
class dimir(commands.Bot):
    ...
    def add_commands(self):
        ...
        async def status(ctx):
            print(ctx)
            await ctx.channel.send(f'{self.message2}, {ctx.author.display_name}')

        @self.command(name = 'hello', pass_context = True)
        async def hello(ctx):
            await ctx.channel.send(f'Hello {ctx.author.display_name}, I am Dimir, I am the doctor for The Banner.')
    ...
    def add_events(self):
        ...
        async def on_message(message):
            if 'dimir' in message.content.lower() and message.author.display_name.lower() != 'dimir':
                await message.channel.send(f'[ERROR://] Hello {message.author.display_name}, I don\'t know how to respond to that. My daddy has not yet completed my training')
